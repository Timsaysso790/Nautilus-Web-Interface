...
export default function StocksPage() {
  ...
  return (
    <AppLayout
      title="Stocks"
      subtitle="Real-time quotes, charts, and watchlist"
    >
        ...
        <div className="lg:col-span-3 space-y-4">
          {!selected && !loadingQuote && (
            <div className="text-center py-16 text-muted-foreground">
              ...
              <p className="text-base">Search for a stock to get started</p>
            </div>
          )}
              ...
              <CardContent className="pt-6">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <div className="flex items-baseline gap-3 mt-2">
                      <span className="tabular-mono text-3xl font-bold text-foreground">
                        {quote.price != null ? formatCurrency(quote.price) : "—"}
                      </span>
                      {quote.change != null && (
                        <span className={`tabular-mono text-lg font-medium ${quote.change >= 0 ? 'text-profit' : 'text-loss'}`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 text-sm">
                  <div>
                    ...
                    <p className="tabular-mono text-foreground font-medium">{formatCurrency(quote.open)}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Prev Close</p>
                    <p className="tabular-mono text-foreground font-medium">{formatCurrency(quote.prev_close)}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Bid / Ask</p>
                    <p className="tabular-mono text-foreground font-medium">{quote.bid != null ? formatCurrency(quote.bid) : "—"} / {quote.ask != null ? formatCurrency(quote.ask) : "—"}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Volume</p>
                    <p className="tabular-mono text-foreground font-medium">{quote.volume != null ? quote.volume.toLocaleString() : "—"}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="tabular-mono text-foreground font-medium">{formatLarge(info.market_cap)}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">P/E Ratio</p>
                    <p className="tabular-mono text-foreground font-medium">{info.pe_ratio != null ? info.pe_ratio.toFixed(2) : "—"}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Dividend Yield</p>
                    <p className="tabular-mono text-foreground font-medium">{info.dividend_yield != null ? `${(info.dividend_yield * 100).toFixed(2)}%` : "—"}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Beta</p>
                    <p className="tabular-mono text-foreground font-medium">{info.beta != null ? info.beta.toFixed(2) : "—"}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">52W High</p>
                    <p className="tabular-mono text-foreground font-medium">{formatCurrency(info["52w_high"])}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">52W Low</p>
                    <p className="tabular-mono text-foreground font-medium">{formatCurrency(info["52w_low"])}</p>
                  </div>
                </div>
                {info.description && (
